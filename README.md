...
- **V-D4RL (DreamerV2 offline pixels)**: pixel trajectories stored as `*.npz` (see upstream [v-d4rl](https://github.com/conglu1997/v-d4rl)). Train with `--override data=[base,vd4rl_walker_random] --override model=vd4rl_dt` and set `paths.data_root` to the folder containing `main/<task>/<split>/<pixel_size>/*.npz`. Details: **[docs/VD4RL_TRAINING.md](docs/VD4RL_TRAINING.md)**.
