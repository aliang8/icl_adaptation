...
- **V-D4RL (offline pixels)**: `*.npz` for 64px (DreamerV2) or `*.hdf5` for 84px (DrQ-v2); see upstream [v-d4rl](https://github.com/conglu1997/v-d4rl). Train with `--override data=[base,vd4rl_walker_random] --override model=vd4rl_dt` and set `paths.data_root` to the folder that directly contains `main/`, `distracting/`, etc., so the leaf path is `.../<suite>/<task>/<split>/<pixel_size>/`. Details: **[docs/VD4RL_TRAINING.md](docs/VD4RL_TRAINING.md)**.
...
- **ManiSkill (ICL HDF5 from PPO)**: `paths.data_root/maniskill/<EnvId>/` with flat v2 ``trajectories_shard_*.h5`` (and optional ``icl_shards_manifest.json``) or a single ``trajectories*.h5``. Loaders discover all matching files; see **[docs/MANISKILL.md](docs/MANISKILL.md)** and ``src/data/ic_replay_buffer_hdf5.py``.
