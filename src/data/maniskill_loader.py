"""Load ManiSkill PPO-exported ``trajectories.h5`` (or legacy ``trajectories.pkl``) for ICL training."""
...
def load_maniskill_trajectories(trajectory_path: str) -> Tuple[List[Dict], List[List[Dict]]]:
    """
    Args:
        trajectory_path: Path to ``trajectories.h5`` (preferred) or legacy ``trajectories.pkl``.
